...
fastify.get("/signin", async function (request, reply) {
  ...
  return `
    ...
    <h2>Token</h2>
    <textarea style="width: 100%; height: 30%; overflow-y: scroll"
    >${token}</textarea>
  `;
});
...
fastify.get("/signedin", async function (request, reply) {
  ...
  try {
    ...
    return `
            ...
            <p>Did you remember to send an Authentication header to this route? check the
            signin route '/signin' for instructions.</p>
            <p> The error message given by the JWT fastify library is "<em>${error}</em>"</p>
        `;
  }
});
